...
Open an image (or drop one in), stack up some edits, watch the preview update live, and save the result. Everything is non-destructive - each edit is replayed from the original, so you can tweak, toggle, or remove any of them at any time. Hold the compare key to peek at the original, and the preview has pan/zoom, fit-to-window, rulers and a cursor readout.
